...
export const DELETE: RequestHandler = async ({ request }) => {
	...
	try {
		if (!body.id) {
			return json({ success: false, error: 'Item ID is required for deletion' }, { status: 400 });
		}

		// Attempt hard deletion first
		const deletedItem = await db.delete(menuItems).where(eq(menuItems.id, body.id)).returning();

		// If nothing was deleted and no error thrown, it means the ID didn't exist
		...
		// Fallback: If it fails due to a Foreign Key Constraint ('23503')
		if (dbError?.cause?.code === '23503') {
			try {
				...
				return json({
					...
					action: 'archived',
					message:
						'Item could not be deleted because it is tied to order history. It has been archived instead.',
					data: archivedItem[0]
				});
			} catch {
				return json({ success: false, error: 'Failed to archive item fallback.' }, { status: 500 });
			}
		}

		// Return a generic error if it fails for any other database reason
		return json({ success: false, error: 'Internal Server Error' }, { status: 500 });
	}
};
