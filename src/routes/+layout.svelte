...
<svelte:head>
    ...
    <meta name="description" content="Manage your campus digital wallet and order food live from BPS Canteen." />
...
<div class="relative mx-auto flex h-dvh max-w-md flex-col overflow-hidden bg-background font-sans">
    ...
    {#if showInstallBanner}
        <div class="absolute bottom-22.5 left-4 right-4 z-50 animate-in slide-in-from-bottom-2 fade-in duration-300">
            <div class="flex items-center justify-between border border-emerald-500/30 bg-card p-3 shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="font-mono text-[9px] tracking-widest text-muted-foreground uppercase">Add to home screen</p>
                    </div>
                </div>
    ...
    <nav class="absolute bottom-0 z-40 w-full border-t border-border bg-background px-2 pt-3 pb-6 supports-[padding:max(0px)]:pb-[max(1.5rem,env(safe-area-inset-bottom))]">
        <div class="mx-auto flex max-w-sm items-center justify-around">
            {#each navItems as { path, icon: Icon, label } (path)}
                {#if path !== resolve('/menu') || !appState.activeTicket}
                    <a
                        href={path}
                        class="relative flex w-16 flex-col items-center gap-1.5 transition-colors {currentPath === path || (path === '/profile' && currentPath.startsWith('/profile')) ? 'text-foreground' : 'text-muted-foreground hover:text-foreground/70'}"
                    >
                        {#if path === resolve('/ticket') && appState.activeTicket}
                            <div class="relative">
                                <Icon size={20} strokeWidth={currentPath === path ? 2 : 1.5} />
                                <span class="absolute -top-0.5 -right-1 h-2 w-2 rounded-full border-2 border-background bg-amber-400"></span>
                            </div>
                        {:else}
                            <Icon size={20} strokeWidth={currentPath === path || (path === '/profile' && currentPath.startsWith('/profile')) ? 2 : 1.5} />
                        {/if}
                        ...
                        {#if currentPath === path || (path === '/profile' && currentPath.startsWith('/profile'))}
                            <span class="absolute -bottom-3 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full bg-foreground"></span>
                        {/if}
                    </a>
                {/if}
            {/each}
